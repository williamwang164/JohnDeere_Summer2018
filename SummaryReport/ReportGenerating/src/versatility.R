...
get.smooth_plot <- function(df, var){
  N <- length(df$time)
...
smooth_field_plot <- function(var, df){
  # data.frame(machines   =  unique(df$machine_name),
  # smoothData = sapply(unique(df$machine_name),
  #                      function(mn){
  #                        get.smooth_plot(df[df$machine_name==mn,], var)
  #                      }))
  
  data1 <- get.smooth_plot(df[df$machine_name=="As Harvested",], var)
  data2 <- get.smooth_plot(df[df$machine_name=="Simulated",], var)
  rbind(data1, data2)
}
...
get.smooth_placHolder <- function(df, var){
  N <- length(df$time)
  data1 <- data.frame(time = as.numeric(df$time) )
  data1$field <- unique(df$field)
  data1[, var]  <- df[[var]]
  data1$machine_name <- df$machine_name
  return(list(data1))
...
smooth_field_placHolder <- function(var, df){
  machines <- unique(df$machine_name)
  data1 <- rbindlist(sapply(machines,
                            function(mn){
                              get.smooth_placHolder(df[df$machine_name==mn,], var)
                            }))
  return(data1)
...
dat.area <- function(var, dat, mc=NA){
  if(is.na(mc)){ mc <- dat.max_capacity(var, dat) }
  round(sum(dat[,c(var)], na.rm=T)/ (60 * 60), 2)
}


machine.area <- function(machine_name, field, var, dat){
  dat.area(var, dat[dat$machine_name == machine_name & dat$field == field,])
}

get_machine_area_noSmooth <- function(field, var, dat){
  smoothed_dat <- as.data.frame(smooth_field_placHolder(var, dat))
  # machines <- unique(dat$machine_name)[order(levels(dat$machine_name))]
  machines <- unique(dat$machine_name)
  
  area <- data.frame(
    machine_name = machines,
    curveArea = mapply(machine.area, machines, field, var, list(smoothed_dat))
  )
  
  asHarvArea <- area %>% dplyr::filter(., machine_name == "As Harvested")
  asHarvArea <- asHarvArea$curveArea 
  
  
  simArea <- area %>% dplyr::filter(., machine_name == "Simulated")
  simArea <- simArea$curveArea
  
  
  return( c(asHarvArea, simArea))
}
get_machine_area <- function(field, var, dat){
  smoothed_dat <- as.data.frame(smooth_field(var, dat))
  # machines <- unique(dat$machine_name)[order(levels(dat$machine_name))]
  machines <- unique(dat$machine_name)
  ...
  asHarvArea <- area %>% dplyr::filter(., machine_name == "As Harvested")
  asHarvArea <- asHarvArea$curveArea 
  asHarvArea <- round(asHarvArea,2)
  
  simArea <- area %>% dplyr::filter(., machine_name == "Simulated")
  simArea <- simArea$curveArea
  simArea <- round(simArea,2)
  
  return( c(asHarvArea, simArea))
}

get_machine_percent <- function(field, var, dat){
  
  area <- get_machine_area(field, var, dat)
  
  asHarvArea <- area[1]
  
  simArea <- area[2]
  
  percent <- format(round((simArea / asHarvArea - 1 ) * 100, 1), nsmall = 3)
  return(percent)
}

get_machine_ratio <- function(field, var, dat){
  area <- get_machine_area(field, var, dat)
  ...
  ratio <- format(round((asHarvArea / simArea ), 3), nsmall = 3)
  return(ratio)
}
...
newParamTable <- function(field, var, dat){
  percent <- get_machine_percent(field, var, dat) 
  area <- get_machine_area(field, var, dat)
  ratio <- get_machine_ratio(field, var, dat)
  harvest_times <- get_machine_harvest_time(field, dat)
  
  
  #Additional parameters Dohn requested
  
  #Harvest Time % = (simTime - asHarvTime) / asHarvTime
  
  simHarvTime <- harvest_times %>% dplyr::filter(., machine_name == "Simulated")
  simHarvTime <- simHarvTime$harvest_time
  
  asHarvTime <- harvest_times %>% dplyr::filter(., machine_name == "As Harvested")
  asHarvTime <- asHarvTime$harvest_time
  
  timePercent <-  format(round(((simHarvTime / asHarvTime) - 1) * 100, 1), nsmall = 3)
  
  additionalParams <- data.frame(timePercent = timePercent, asHarvArea = area[1] , simArea = area[2], percent = percent, ratio = ratio)
  names(additionalParams) <- c("Harvest Time %", "As Harvest Area", "Simulated Area","Versatility %", "Versatility Ratio")
  row.names(additionalParams) <- NULL
  
  tbl_c <- kable(additionalParams, "latex", booktabs = TRUE)
